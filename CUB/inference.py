...
import copy, sys, torch, joblib, argparse, random
import numpy as np
...
def eval(args, seed):
    ...
    return class_acc_meter, attr_acc_meter, all_class_labels, topk_class_outputs, all_class_logits, all_attr_labels, all_attr_outputs_wr, all_attr_outputs_sigmoid_wr, wrong_idx, all_attr_outputs2
...
def eval(args):
    ...
    if args.model_dir2:
        if 'rf' in args.model_dir2:
            model2 = joblib.load(args.model_dir2).cuda()
        else:
            model2 = torch.load(args.model_dir2).cuda()
        if not hasattr(model2, 'use_relu'):
            if args.use_relu:
                model2.use_relu = True
            else:
...
# python3 CUB/inference.py -model_dirs CUB/outputs/jointModel/best_model_1.pt -log_dir CUB/output/jointModel -eval_data test -use_attr -data_dir CUB_processed/class_attr_data_10 -feature_group_results 
# python3 CUB/inference.py -model_dirs CUB/outputs/conceptModel/best_model_1.pt -model_dirs2 CUB/outputs/labelModel/best_model_1.pt -eval_data test -use_attr -n_attributes 112 -data_dir CUB_processed/class_attr_data_10 -bottleneck -use_sigmoid
